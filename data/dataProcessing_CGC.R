...
library(plyr)
library(doParallel)
source("func_dataProcessing.R")

set.seed(346)

### IMPORT ###

mix <- read.delim("AntibodyRepertoireCV+RSV+EBOV+YFV.txt") ## input the filepath for the raw dataset, refer to README.txt for sourcing.
...
# save clean day 0 datasets
write_delim(data$covid,"clean_noCGC/covid-clean-0.txt")
write_delim(data$rsv,"clean_noCGC/rsv-clean-0.txt")
...
### SAVE ###
write.table(data.clean$covid, file = "processed/covid-cg.txt", sep = "\t", row.names = FALSE)
write.table(data.clean$rsv, file = "processed/rsv-cg.txt", sep = "\t", row.names = FALSE)

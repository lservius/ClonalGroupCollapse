...
for (i in which.i){
  ...
  xval[i,] <- c("COVID","SVM",i,db,sen,spec,uar)           # save performance metrics
  SVM_hyPar[i,] <- c("COVID","SVM",i, hp_f)                # save hyperparameters
  
  # checkpoint
  write.csv(xval[i,] , paste0("output/", Sys.Date(),".",i,".CP_SVM(xNIC).csv"), row.names = FALSE)
  write.csv(SVM_hyPar[i,] , paste0("output/", Sys.Date(),".",i,".CP_SVMhyPar(xNIC).csv"), row.names = FALSE)

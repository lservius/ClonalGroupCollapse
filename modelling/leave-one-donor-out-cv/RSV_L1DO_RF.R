...
for(i in 1:n_s){
  ...
  # save out xval
  xval[i,] <- data.frame(data = "RSV", model= "RF", iteration = i, data_balance = db, sensitivity = sen, specificity = spec, uar = uar)
  RF_hyPar[i,] <- c("RSV","RF",i, hp_f)                          # save hyperparameter
}
...
write.csv(xval, file = paste0("output/", Sys.Date(), ".RSV_DvsD_RF(xNIC).csv"))
write.csv(RF_hyPar, file = paste0("output/", Sys.Date(), ".RSV_hyPar_DvsD_RF(xNIC).csv"))

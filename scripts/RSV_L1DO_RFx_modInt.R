#"RSV (CG): RF-NSx for (d-1)vs1"

print("Aim: Use RF on RSV for performance and model comparison but now the data has 1 observation = 1 clonal group. While using default hyperparameters for all except nodesize which will be manually varied.")
...
# save out xval
xval <- data.frame(data = "RSV", model = "RF-NSx",data_balance = db, sensitivity = sen, specificity = spec, uar = uar, g.uar = g.uar, AUC_ROC = auc_roc, Precision = cm$byClass[[5]], Recall = cm$byClass[[6]], F1 = cm$byClass[[7]]) 
...
write.csv(xval, file = "../results/figures_tables/table3_RSV_RFx.csv")
write.csv(RF_hyPar, file = "../results/RSV_modInt_L1DO_hyPar_RFx.csv")

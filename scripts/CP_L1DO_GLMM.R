...
xval <- data.frame(model = "GLMM", data = "COVID", iteration = 1, data_balance = 1, sensitivity = 1, specificity = 1, uar = 1, g.uar = 1, AUC_ROC = 1, Precision = 1, Recall = 1, F1 = 1) 
...
system.time(
  xval_glmm <- foreach(i=1:n_s,
                       .packages = c('caret','tidyverse','glmnet')) %dopar% {
                         ...
                         cat("Iteration:", i, "\n")
                         print(coef(model))
                         saveRDS(model, file = paste0(Sys.Date(),".i",i, ".COVID_L1DO_GLMM.rda"))
                         ...
                         # save out xval
                         xval <- data.frame(model = "GLMM", data = "COVID", iteration = i,  data_balance = db, sensitivity = sen, specificity = spec, uar = uar, g.uar = g.uar, AUC_ROC = auc_roc, Precision = cm$byClass[[5]], Recall = cm$byClass[[6]], F1 = cm$byClass[[7]]) 
...
###### PRINT RESULTS ######
xval
write.csv(xval, file = "../results/COVID_L1DO_GLMM.csv")
